          ...
          struct sockaddr_in *from, struct sockaddr_in *to)
{
    ...
    diag_printf("%4d bytes from %s: ", len, inet_ntoa(from->sin_addr));
    diag_printf("icmp_seq=%3d", icmp->icmp_seq);
    diag_printf(", time=%2dms\n", (int)(tv - *tp)*10);
...
ping_host(int s, struct sockaddr_in *host)
{
	...
	if ((pkt1 = malloc(MAX_PACKET)) == NULL ||
		...
		diag_printf("Error: Unable to allocate memory!");
		goto badexit;
	}
...
CMD_DECL(ping_cmd)
{
    ...
    if (!inet_aton((char *)argv[0], &host.sin_addr)) {
		diag_printf("Invalid address!");
		return SHELL_OK;
	}

    if ((p = getprotobyname("icmp")) == (struct protoent *)0) {
        diag_printf("Error: getprotobyname\n");
        return SHELL_OK;
    }

    s = socket(AF_INET, SOCK_RAW, p->p_proto);
    if (s < 0) {
        diag_printf("Error: opening socket\n");
        return SHELL_OK;
    }
