...
//	long long int block = 31997250 * 4096; //_val(argv[1]);
	long long int block = 131060736000; //_val(argv[1]);

	lseek(f, block , SEEK_CUR);
	
	block = 0 ;

	block = lseek(f, 0 , SEEK_CUR) ;

	printf ("%llu\n", block);

	//lseek(f, 131060736000, SEEK_CUR);
	...
	printf ("%x %x %x %d %x\n", in.i_atime, in.i_ctime, in.i_mtime, in.i_size_lo, in.i_mode);
#endif
