...
//	revised on: 16 AUG 2007 -- add '/proc/vram' pseudo-file info
//	revised on: 15 OCT 2007 -- for kernel 2.6.22 with ATI Radeon
//-------------------------------------------------------------------

#include <linux/module.h>	// for init_module() 
#include <linux/pci.h>		// for pci_find_class() 
#include <linux/proc_fs.h>	// for create_ptoc_info_entry() 

//#define VENDOR_ID 0x1039	// Silicon Integrated Systems
//#define DEVICE_ID 0x6325	// SiS-315 Graphics Processor

#define VENDOR_ID 0x8086	// Advanced Techlologies, Inc
#define DEVICE_ID 0x0102	// Radeon X300 Graphics Processor 

//#define VENDOR_ID 0x8086	// Intel Corporation, Inc
//#define DEVICE_ID 0x2772	// 82945G Graphics Processor 

//#define VENDOR_ID 0x102B	// Matrox Corporation, Inc
//#define DEVICE_ID 0x0525	// G400 Graphics Processor 

char device_name[] = " ATI Radeon X300 Graphics Controller";
...
struct pci_dev	*devp;		// pointer to PCI device-structure
...
int my_get_info( char *buf, char **start, off_t off, int count )
{
	...
	len += sprintf( buf+len, "\n %s -- ", device_name );
	len += sprintf( buf+len, "PCI Configuration Space Header \n" );
	for (i = 0; i < 256; i+=4)
		{
		if ( (i % 32) == 0 ) 
			len += sprintf( buf+len, "\n%02X: ", i );
